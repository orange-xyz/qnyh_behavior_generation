...
class Generator(object):
    def __init__(self, num_vocabulary, batch_size, emb_dim, hidden_dim,
        ...
        with tf.variable_scope('generator'):
            ...
            if self.ADDDATA == True:
                self.g_embeddings = tf.Variable(self.init_matrix([self.num_vocabulary, self.emb_dim - self.class_dim - self.grade_dim]))
        ...
        #self.g_start_token = tf.placeholder(tf.int32, shape=[self.batch_size])
        # self.g_x = tf.placeholder(tf.int32, shape=[self.batch_size,
        ...
        gen_length = 20
        gen_o = tensor_array_ops.TensorArray(dtype=tf.float32, size=gen_length,
                                             dynamic_size=False, infer_shape=True)
        gen_x = tensor_array_ops.TensorArray(dtype=tf.int32, size=gen_length,
                                             dynamic_size=False, infer_shape=True)

        # g_ta_emb_x = tensor_array_ops.TensorArray(
        #     dtype=tf.int32, size=self.sequence_length)
        # g_ta_emb_x = g_ta_emb_x.unstack(self.g_processed_x)
        ...
        def _g_recurrence(i, x_t, h_tm1, gen_o, gen_x):
            ...
            # x_tp1 = tf.cond(tf.cast(self.ADDDATA, tf.bool), lambda: tf.concat([tf.nn.embedding_lookup(self.g_embeddings, next_token), ta_emb_context.read(i)], axis=1),
            #                 lambda: tf.cast(tf.nn.embedding_lookup(self.g_embeddings, next_token), tf.float32))
